...
def score_diarization(ref_rttm_files, sys_rttm_files, uem_file=None, collar=0.0,
                      ignore_overlaps=False, jer_min_ref_dur=0.0, step=0.010,
                      n_digits=2, table_format='simple', case_name=''):
    """Hàm chính để tính toán các chỉ số từ RTTM.

    Parameters:
        ref_rttm_files (list): Danh sách các file RTTM tham chiếu.
        sys_rttm_files (list): Danh sách các file RTTM hệ thống.
        uem_file (str): File UEM (optional).
        collar (float): Kích thước collar khi tính DER.
        ignore_overlaps (bool): Bỏ qua overlapped speech khi tính DER.
        jer_min_ref_dur (float): Thời lượng tối thiểu của JER.
        step (float): Khoảng bước thời gian (ms).
        n_digits (int): Số chữ số thập phân.
        table_format (str): Định dạng bảng kết quả.

    Returns:
        dict: Kết quả gồm các chỉ số tính toán.
    """
    # Load speaker/reference speaker turns and UEM
    ref_turns, _ = load_rttms(ref_rttm_files)
    sys_turns, _ = load_rttms(sys_rttm_files)
    if uem_file:
        uem = load_uem(uem_file)
    else:
        uem = gen_uem(ref_turns, sys_turns)

    # Trim and merge speaker turns
    ref_turns = merge_turns(trim_turns(ref_turns, uem))
    sys_turns = merge_turns(trim_turns(sys_turns, uem))

    # Scoring
    file_scores, global_scores = score(ref_turns, sys_turns, uem, step=step,
                                       jer_min_ref_dur=jer_min_ref_dur,
                                       collar=collar, ignore_overlaps=ignore_overlaps)

    # Print results and return scores as a dictionary
    return print_table(file_scores, global_scores, n_digits, table_format, case_name)
